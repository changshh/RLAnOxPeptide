    ...
    for epoch in range(args.num_epochs):
        ...
        if sup_dataloader and (epoch + 1) % args.sup_train_freq == 0:
            for input_ids, labels in tqdm(sup_dataloader, desc=f"Epoch {epoch+1} SL"):
                ...
                loss.backward()
                optimizer.step()
                sl_loss_accum += loss.item()
                sl_loss_count += 1

        avg_rl_loss = rl_loss_accum / max(1, rl_loss_count)
        print(f"  [Loss Epoch {epoch+1}] RL Avg Loss={avg_rl_loss:.4f}")
        if sl_loss_count > 0:
            avg_sl_loss = sl_loss_accum / sl_loss_count
            print(f"  [Loss Epoch {epoch+1}] SL Avg Loss={avg_sl_loss:.4f}")
